...
DBaseInterface::DBaseInterface():
    user("root"),server("127.0.0.1"),database("jp"),password("maverick"),
    t(server.c_str(),user.c_str(),database.c_str(),
		password.c_str())
{
// 	char query[256];
// 
...
void DBaseInterface::Query(string str)
{
  t.Query(str.c_str());
  result = t.Fetch();
  while(!result.empty())
	...
	float speed)
{
	...
	string query = "INSERT INTO GPS_DATA (Longitude, Lattitude, Speed) VALUES(";
